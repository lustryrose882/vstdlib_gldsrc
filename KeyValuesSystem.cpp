...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CKeyValuesSystem::CKeyValuesSystem() : m_Strings(1024),
	m_HashItemMemPool(sizeof(hash_item_t), 64, CMemoryPool::GROW_FAST, "CKeyValuesSystem::m_HashItemMemPool"),
	m_KeyValuesTrackingList(0, 0, &CKeyValuesSystem::MemoryLeakTrackerLessFunc)
{
...
HKeySymbol CKeyValuesSystem::GetSymbolForString( const char *name )
{
	...
	while (1)
	{
		if (!stricmp(name, (char *)m_Strings.Base()[item->stringIndex] ))
		{
		...
		if (item->next == NULL)
		{
			...
			item->stringIndex = m_Strings.Count();
